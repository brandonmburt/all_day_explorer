...
export const generateTeamObjMapByType = (playTypes: string[], teams: Map<string, string>): [Map<string, { [key: string]: number }>, { [key: string]: number }] => {

    const typesObj: { [key: string]: number } = { total: 0 };
    playTypes.forEach(type => typesObj[type] = 0);

    const teamObjMap: Map<string, { [key: string]: number }> = new Map();
...
export const generateTeamObjMapByTier = (tiers: string[], teams: Map<string, string>): [Map<string, { [key: string]: number }>, { [key: string]: number }] => {

    const tiersObj: { [key: string]: number } = { total: 0 };
...
export const isMobile = (): boolean => {
    ...
    const mobile = window.mobileCheck = function () {
        let check = false;
